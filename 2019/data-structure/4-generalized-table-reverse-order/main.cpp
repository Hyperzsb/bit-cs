...
GLNode *reverse(GLNode *p) {
    if (p == NULL) {
        ...
        if (p->tag == ATOM) {
            ...
            GLNode *current_node = p;
            current_node->tp = reverse(current_node->tp);
            while (current_node->tp != NULL) {
                current_node = current_node->tp;
            }
            current_node->tp = (GLNode *) malloc(sizeof(GLNode));
            current_node->tp->tag = p->tag;
            current_node->tp->tp = NULL;
            current_node->tp->ptr.hp = reverse(p->ptr.hp);
            return p->tp;
        }
    }
}