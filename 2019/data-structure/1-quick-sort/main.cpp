...
void QuickSort(int *node, int start, int end) {
    if (end - start < 4) {
        ...
        if ((node[start] < node[center] && node[center] < node[end]) ||
            (node[start] > node[center] && node[center] > node[end]))
            Swap(node, center, start);
        if ((node[start] < node[end] && node[end] < node[center]) ||
            (node[start] > node[end] && node[end] > node[center]))
            Swap(node, end, start);
