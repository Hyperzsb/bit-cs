...
int main(){
	...
	for(int i=0;i<zuShu;i++){
		...
		for(int j=0;j<length[1-flag];j++){
			if((num[flag][length[flag]-1-j]-'0')+(num[1-flag][length[1-flag]-1-j]-'0')+jinWei==0){
				resList[length[flag]-j]='0';
				jinWei=0;
				continue;
			}
			if((num[flag][length[flag]-1-j]-'0')+(num[1-flag][length[1-flag]-1-j]-'0')+jinWei==1){
				resList[length[flag]-j]='1';
				jinWei=0;
				continue;
			}
			if((num[flag][length[flag]-1-j]-'0')+(num[1-flag][length[1-flag]-1-j]-'0')+jinWei==2){
				resList[length[flag]-j]='0';
				jinWei=1;
				continue;
			}
			if((num[flag][length[flag]-1-j]-'0')+(num[1-flag][length[1-flag]-1-j]-'0')+jinWei==3){
				resList[length[flag]-j]='1';
				jinWei=1;
				continue;
			}
