		...
		void GetTrueValue() {
			...
			for (int i = 0; i < this->value_num; i++)
				if (true_value[i] == 1)
					true_value_num++;
			if (true_value_num == 0)
				this->proposition_type = Contradiction;
			else if (true_value_num == value_num)
				this->proposition_type = Tautology;
			else
				this->proposition_type = Contingency;
			return ;
		}
...
int main() {
	...
	PropositionType proposition_type = proposition->GetPropositionType();
	if (proposition_type == Contradiction)
		printf("NO\n");
	else
		printf("YES\n");
	return 0;
