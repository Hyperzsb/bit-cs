...
int main(){
	...
	for(int i=0;i<2;i++){
		...
		for(int j=1;j<=matrix[i].size;j++){
			scanf("%d%d%d",&matrix[i].element_list[j].column,&matrix[i].element_list[j].row,&matrix[i].element_list[j].value);
		}
	}
	...
	for(int i=1;i<=matrix[0].size;i++){
		...
		for(int j=1;j<=matrix[1].size;j++){
			if(matrix[0].element_list[i].row==matrix[1].element_list[j].column){
				tmp_buffer[matrix[1].element_list[j].row]+=matrix[0].element_list[i].value*matrix[1].element_list[j].value;
			}
		}
	}
	...
	for(int i=0;i<sum;i++){
		printf("%d,%d,%d\n",matrix[2].element_list[i].column,matrix[2].element_list[i].row,matrix[2].element_list[i].value);
	}
	return 0;
}
