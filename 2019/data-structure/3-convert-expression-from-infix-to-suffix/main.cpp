...
int main() {
    ...
    while (zuShu--) {
        ...
        char scentence[1000] = {'\0'};
        scanf("%s", scentence);
        for (int i = 0; scentence[i] != '#'; i++) {
            if ((scentence[i] >= 'a' && scentence[i] <= 'z') || (scentence[i] >= 'A' && scentence[i] <= 'Z')) {
                Data *new_num = new Data(scentence[i], '!');
                num_queue->Push(*new_num);
                delete new_num;
            } else {
                Data *new_oper = new Data('!', scentence[i]);
